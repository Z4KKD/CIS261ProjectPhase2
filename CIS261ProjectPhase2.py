...
def GetDatesWorked():
    ...
    while True:
            ...
            if fromdate == "":
                fromdate = print("Please Enter Start Date (mm/dd/yyyy)")
                continue
            else: break
    while True:
        todate = input("Enter End Date (mm/dd/yyyy): ")
        if todate == "":
                todate = print("Please Enter End Date (mm/dd/yyyy)")
                continue
        else: break
...
def PrintTotals(EmpTotals, TotalHrs, TotGrossPay, TotTax, TotNetPay):    
    print()
    ...
    print(f"Total Hours Worked: {TotalHrs:,.2f}")
    print(f"Total Gross Pay: {TotGrossPay:,.2f}")
    print(f"Total Income Tax:  {TotTax:,.2f}")
    print(f"Total Net Pay: {TotNetPay:,.2f}")
...
if __name__ == "__main__":
    ...
    PrintTotals (EmpTotals,EmpTotals["TotHours"],EmpTotals["TotGrossPay"], EmpTotals["TotTax"], EmpTotals["TotNetPay"])